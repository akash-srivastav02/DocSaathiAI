...
export default function PDFCompressPage() {
  ...
  // Target size state
  const [targetValue, setTargetValue] = useState("");   // raw number input
  const [targetUnit, setTargetUnit]   = useState("KB"); // "KB" or "MB"
  ...
  // Convert user input to KB for the API
  const getTargetKB = () => {
  ...
  const handleFile = (f) => {
    ...
    // Strict PDF check — by extension AND mimetype
  ...
  const handleCompress = async () => {
    ...
    // Warn if target is unrealistically small
    const originalKB = Math.round(file.size / 1024);
    if (targetKB > originalKB) {
      setError(`Your file is already ${originalKB} KB — smaller than your target of ${targetKB} KB. No compression needed.`);
      return;
    }
    ...
    <div style={s.root}>
      ...
      <div style={s.main}>
        ...
        <div style={s.card}>
          ...
          <p style={s.hint}>
            Enter the size you want the PDF to be compressed to.
            {fileSizeKB > 0 && <span style={{ color: "#f97316" }}> Your file is currently <b>{fileSizeKB} KB</b>.</span>}
          </p>
          ...
          {/* Show what target translates to */}
          {targetKB && targetKB > 0 && (
            <p style={s.targetHint}>
              ...
              {fileSizeKB > 0 && targetKB < fileSizeKB && (
                <span style={{ color: "#22c55e" }}>
                  &nbsp;→ need to reduce by {Math.round(((fileSizeKB - targetKB) / fileSizeKB) * 100)}%
                </span>
              )}
            </p>
          )}
          ...
          <div style={s.resultCard}>
            ...
            <div style={s.statsRow}>
              ...
              <div style={s.arrowCol}>
                ...
                <span style={{
                  ...s.reductionPill,
                  background: result.reduction >= 20 ? "#052e16" : "#1c1917",
                  color:      result.reduction >= 20 ? "#86efac" : "#fca57a",
                  border:     `1px solid ${result.reduction >= 20 ? "#14532d" : "#78350f"}`,
                }}>
                  -{result.reduction}%
                </span>
              </div>
            ...
            {/* ── Key message: hit or miss ────────────────────────────── */}
            {result.hitTarget ? (
              <div style={s.msgSuccess}>
                ✅ Successfully compressed to <b>{result.compressedKB} KB</b> — within your target of <b>{result.targetKB} KB</b>.
              </div>
            ) : (
              <div style={s.msgWarning}>
                ⚠️ <b>This PDF cannot be compressed below {result.compressedKB} KB.</b>
                <br /><br />
                Your target was <b>{result.targetKB} KB</b> but the best we could achieve is <b>{result.compressedKB} KB</b>.
                <br /><br />
                <b>Why?</b> Text-based PDFs (created digitally) are already very efficiently encoded.
                Unlike scanned PDFs (photos of documents), they cannot be compressed much further
                without destroying the text. You can still download this compressed version.
              </div>
            )}
...
const s = {
  ...
  unitBtn:    { border: "none", padding: "12px 20px", fontSize: 15, fontWeight: 700, cursor: "pointer", transition: "all 0.15s" },
