...
const NAV_ITEMS = [
  ...
  { label: "Vault",     icon: "🗄️",  route: "/vault"     },
  { label: "Pricing",   icon: "💳", route: null          },
...
export default function Sidebar({ credits, activeNav, setActiveNav, setShowPricing, onLogout }) {
  ...
  const handleNav = (item) => {
    ...
    if (item.label === "Pricing") { setShowPricing(true); navigate("/dashboard"); }
    else if (item.route) { setShowPricing(false); navigate(item.route); }
  };
  ...
  return (
    <div style={s.sidebar}>
      ...
      <div style={s.creditBlock}>
        ...
        <button style={s.buyBtn} onClick={() => { setShowPricing(true); navigate("/dashboard"); }}>
          + Buy Credits
        </button>
      </div>
      ...
      <nav style={s.nav}>
        {NAV_ITEMS.map((item) => (
          <button
            key={item.label}
            style={{ ...s.navBtn, ...(activeNav === item.label ? s.navActive : {}) }}
...
const s = {
  ...
  brandText: { display: "flex", alignItems: "baseline" },
  ...
  navBtn: { display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", border: "none", background: "transparent", color: "#64748b", borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 500, width: "100%", transition: "all 0.15s" },
